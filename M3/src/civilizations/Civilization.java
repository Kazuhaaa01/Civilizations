...
public class Civilization implements Variables {
		...
		if (wood >= totalWoodCost && food >= totalFoodCost && iron >= totalIronCost) {
			...
			for (int i = 0; i < n; i++) {
				int armor = ARMOR_SWORDSMAN
						+ (ARMOR_SWORDSMAN * PLUS_ARMOR_SWORDSMAN_BY_TECHNOLOGY * technologyDefense) / 100;
				int baseDamage = BASE_DAMAGE_SWORDSMAN
						+ (BASE_DAMAGE_SWORDSMAN * PLUS_ATTACK_SWORDSMAN_BY_TECHNOLOGY * technologyAtack) / 100;
		...
		if (wood >= totalWoodCost && food >= totalFoodCost && iron >= totalIronCost) {
			...
			for (int i = 0; i < n; i++) {
				int armor = ARMOR_SPEARMAN
						+ (ARMOR_SPEARMAN * PLUS_ARMOR_SPEARMAN_BY_TECHNOLOGY * technologyDefense) / 100;
				int baseDamage = BASE_DAMAGE_SPEARMAN
						+ (BASE_DAMAGE_SPEARMAN * PLUS_ATTACK_SPEARMAN_BY_TECHNOLOGY * technologyAtack) / 100;
		...
		if (wood >= totalWoodCost && food >= totalFoodCost && iron >= totalIronCost) {
			...
			for (int i = 0; i < n; i++) {
				int armor = ARMOR_CROSSBOW
						+ (ARMOR_CROSSBOW * PLUS_ARMOR_CROSSBOW_BY_TECHNOLOGY * technologyDefense) / 100;
				int baseDamage = BASE_DAMAGE_CROSSBOW
						+ (BASE_DAMAGE_CROSSBOW * PLUS_ATTACK_CROSSBOW_BY_TECHNOLOGY * technologyAtack) / 100;
		...
		if (wood >= totalWoodCost && food >= totalFoodCost && iron >= totalIronCost) {
			...
			for (int i = 0; i < n; i++) {
				...
				int baseDamage = BASE_DAMAGE_CANNON
						+ (BASE_DAMAGE_CANNON * PLUS_ATTACK_CANNON_BY_TECHNOLOGY * technologyAtack) / 100;
		...
		if (wood >= totalWoodCost && food >= totalFoodCost && iron >= totalIronCost) {
			...
			for (int i = 0; i < n; i++) {
				int armor = ARMOR_ARROWTOWER
						+ (ARMOR_ARROWTOWER * PLUS_ARMOR_ARROWTOWER_BY_TECHNOLOGY * technologyDefense) / 100;
				int baseDamage = BASE_DAMAGE_ARROWTOWER
						+ (BASE_DAMAGE_ARROWTOWER * PLUS_ATTACK_ARROWTOWER_BY_TECHNOLOGY * technologyAtack) / 100;
		...
		if (wood >= totalWoodCost && food >= totalFoodCost && iron >= totalIronCost) {
			...
			for (int i = 0; i < n; i++) {
				int armor = ARMOR_CATAPULT
						+ (ARMOR_CATAPULT * PLUS_ARMOR_CATAPULT_BY_TECHNOLOGY * technologyDefense) / 100;
				int baseDamage = BASE_DAMAGE_CATAPULT
						+ (BASE_DAMAGE_CATAPULT * PLUS_ATTACK_CATAPULT_BY_TECHNOLOGY * technologyAtack) / 100;
			...
			for (int i = 0; i < n; i++) {
				int armor = ARMOR_ROCKETLAUNCHERTOWER
						+ (ARMOR_ROCKETLAUNCHERTOWER * PLUS_ARMOR_ROCKETLAUNCHERTOWER_BY_TECHNOLOGY * technologyDefense)
								/ 100;
				int baseDamage = BASE_DAMAGE_ROCKETLAUNCHERTOWER + (BASE_DAMAGE_ROCKETLAUNCHERTOWER
						* PLUS_ATTACK_ROCKETLAUNCHERTOWER_BY_TECHNOLOGY * technologyAtack) / 100;
		...
		if (wood >= totalWoodCost && food >= totalFoodCost && iron >= totalIronCost) {
			...
			for (int i = 0; i < n; i++) {
				...
				int baseDamage = BASE_DAMAGE_MAGICIAN
						+ (BASE_DAMAGE_MAGICIAN * PLUS_ATTACK_MAGICIAN_BY_TECHNOLOGY * technologyAtack) / 100;
	...
	public ArrayList<MilitaryUnit> getArmy() {
		...
		// Reunimos las tropas de cada casilla en una sola lista.
	...
	public void updateArmy(ArrayList<MilitaryUnit> supervivientes) {
		...
		// Primero vaciamos el ejercito actual para reconstruirlo desde cero.
		for (int i = 0; i < this.army.length; i++) {
			if (this.army[i] != null) {
				this.army[i].clear();
			} else {
